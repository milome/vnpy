...
from vnpy.chart.base import UP_COLOR, DOWN_COLOR, PEN_WIDTH, BAR_WIDTH
...
class CrossIndexCandleItem(ChartItem):
    ...
    def __init__(
        self, 
        manager: BarManager,
        base_manager: Optional[BarManager] = None,
        bearish_fill_opacity: float = 0.05,
        bullish_color: Optional[QtGui.QColor] = None,
        bearish_color: Optional[QtGui.QColor] = None,
        ...
        self._base_manager: Optional[BarManager] = base_manager
        ...
        self._bullish_brush: QtGui.QBrush = QtGui.QBrush(QtCore.Qt.NoBrush)  # 空心
        
        # 阴线：默认青色
        # 要求：引线和实体边界完全显示（不透明），只有填充区域高透明度（可调）
        # bearish_fill_opacity: 阴线实体填充的不透明度（0.0-1.0），默认0.05 ≈ 95%透明
        self._bearish_fill_opacity: float = max(0.0, min(1.0, bearish_fill_opacity))
    ...
    def _update_bearish_brush(self) -> None:
        ...
        根据当前的不透明度设置更新阴线填充画刷。
        """
        bearish_brush_color = QtGui.QColor(*DOWN_COLOR)
        # setAlphaF 接受 0.0-1.0 之间的不透明度（1.0 为完全不透明）
        bearish_brush_color.setAlphaF(self._bearish_fill_opacity)
        self._bearish_brush: QtGui.QBrush = pg.mkBrush(color=bearish_brush_color)
    ...
    def set_bearish_fill_opacity(self, opacity: float) -> None:
        ...
        """
        self._bearish_fill_opacity = max(0.0, min(1.0, opacity))
        self._update_bearish_brush()
